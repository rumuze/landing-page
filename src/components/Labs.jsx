...
const Labs = () => {
  ...
  return (
    <div className="min-h-screen bg-[#00050a] font-mono selection:bg-cyan selection:text-background overflow-hidden relative">
      <SEO 
      ...
      <nav className="p-6 relative z-10 flex justify-between items-center max-w-7xl mx-auto">
        ...
        <div className="text-[10px] text-white/40 tracking-[0.3em]">
      ...
      <main className="max-w-7xl mx-auto px-6 py-20 relative z-10">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {experiments.map((exp, idx) => (
            <motion.div
              ...
              className="group border border-white/5 bg-white/[0.02] p-8 hover:bg-white/[0.05] hover:border-cyan/30 transition-all duration-500 relative overflow-hidden"
            >
              <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-100 transition-opacity">
                <Code2 size={40} />
              </div>
              <div className="w-12 h-12 flex items-center justify-center bg-white/5 rounded-lg mb-6 group-hover:scale-110 transition-transform">
                {exp.icon}
              </div>
              ...
              <div className="flex justify-between items-center text-[10px] tracking-widest">
                ...
                <button className="text-white hover:text-cyan flex items-center gap-1 transition-colors">
        ...
        >
          <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-12">
            <div>
              <h2 className="text-2xl font-bold mb-2 tracking-tight">LATEST PUBLICATIONS</h2>
              <p className="text-sm text-white/40">Technical deep-dives into our core research.</p>
            </div>
            <button className="text-xs border border-white/10 px-6 py-2 hover:bg-white/5 tracking-[0.2em] transition-colors">
              VIEW ARCHIVE
            </button>
          </div>
          ...
          <div className="space-y-4">
             {[1, 2].map(i => (
               <div key={i} className="flex justify-between items-center p-6 border border-white/5 bg-white/[0.01] hover:bg-white/[0.03] transition-colors group cursor-pointer">
                 <div className="flex items-center gap-6">
                    <span className="text-white/20 text-sm">0{i}</span>
                    <div>
                       <h4 className="font-bold text-white/80 group-hover:text-cyan transition-colors">Recursive Neural Optimization in Distributed Environments</h4>
                       <span className="text-[10px] text-white/40">RELEASED: 12.01.2026 // READ TIME: 12 MIN</span>
                    </div>
                 </div>
                 <ArrowUpRight className="text-white/20 group-hover:text-cyan transition-all group-hover:translate-x-1 group-hover:-translate-y-1" />
               </div>
             ))}
          </div>
        </motion.div>
