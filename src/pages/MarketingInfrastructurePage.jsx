...
const S = {
  ...
  intro: { en: 'Rumuze engineers performance marketing infrastructure systems for organizations that require server-side attribution, governed campaign execution, and weekly KPI accountability with documented reporting.', ar: 'روموز تهندس أنظمة البنية التحتية للتسويق الأدائي للمنظمات التي تتطلب إسناداً من جانب الخادم وتنفيذ حملات محكوماً ومساءلة مؤشرات أداء رئيسية أسبوعية مع تقارير موثقة.' },
  definition: { en: 'Performance marketing infrastructure is the systems layer that makes paid advertising attributable, governable, and continuously improvable. It encompasses server-side tracking architecture, multi-touch attribution modeling, conversion-optimized funnel engineering, CRM lead routing, and reporting pipelines that connect ad spend to revenue outcomes. It is distinct from campaign management, which addresses tactical ad execution only.', ar: 'البنية التحتية للتسويق الأدائي هي طبقة الأنظمة التي تجعل الإعلانات المدفوعة منسوبة وقابلة للحوكمة والتحسين المستمر. تشمل معمارية التتبع من جانب الخادم ونمذجة الإسناد متعدد اللمس وهندسة القمع المحسّن للتحويل وتوجيه العملاء المحتملين عبر CRM وخطوط أنابيب التقارير التي تربط الإنفاق الإعلاني بنتائج الإيرادات.' },
  definitionQ: { en: 'What is Performance Marketing Infrastructure?', ar: 'ما هي البنية التحتية للتسويق الأدائي؟' },
  ...
  sections: [
    {
      ...
      body: { en: 'Accurate revenue attribution is the foundation of performance marketing. Rumuze rebuilds attribution from the tracking layer — not the reporting layer. Implementation includes server-side GTM, Meta Conversion API (CAPI), GA4 with custom event taxonomy, and multi-touch attribution modeling in Looker Studio. Last-click attribution is not a governance model. It is a measurement gap. Organizations also investing in organic visibility benefit from aligning attribution with their SEO infrastructure for unified pipeline measurement.', ar: 'الإسناد الدقيق للإيرادات هو أساس التسويق الأدائي. روموز تعيد بناء الإسناد من طبقة التتبع — وليس طبقة التقارير. التنفيذ يشمل GTM من جانب الخادم وMeta Conversion API (CAPI) وGA4 مع تصنيف أحداث مخصص ونمذجة إسناد متعدد اللمس في Looker Studio. إسناد النقرة الأخيرة ليس نموذج حوكمة. المنظمات التي تستثمر أيضاً في الظهور العضوي تستفيد من مواءمة الإسناد مع بنيتها التحتية لـ SEO.' },
    ...
    },
    {
      ...
      body: { en: 'Rumuze structures campaigns before any budget is allocated. Campaign architecture defines: audience segmentation framework, ad set hierarchy, bid strategy per objective, creative variant rotation schedule, and A/B testing cadence with minimum statistical validity thresholds enforced before creative changes proceed. Google Ads and Meta Ads campaigns are governed by documented ROAS targets and cost-per-qualified-lead benchmarks agreed before launch. No campaign runs on an undefined budget against undefined targets.', ar: 'روموز تهيكل الحملات قبل تخصيص أي ميزانية. معمارية الحملة تحدد: إطار تقسيم الجمهور وتسلسل مجموعات الإعلانات واستراتيجية العطاء لكل هدف وجدول تدوير الإصدارات الإبداعية وإيقاع A/B مع حدود صلاحية إحصائية دنيا مفروضة قبل المتابعة بتغييرات إبداعية. حملات Google Ads وMeta Ads محكومة بأهداف ROAS موثقة ومعايير تكلفة لكل عميل مؤهل متفق عليها قبل الإطلاق.' },
  ...
  faqs: [
    { q: { en: 'How is marketing spend governed across performance engagements?', ar: 'كيف يُحكَم الإنفاق التسويقي عبر ارتباطات الأداء؟' }, a: { en: 'All marketing spend operates under a governance model defined before any budget is deployed. KPIs — including ROAS targets and CPL benchmarks — are agreed in writing. Weekly reports compare actual performance against targets by channel. Underperforming channels are adjusted or paused at agreed review intervals with documented rationale. Budget reallocation requires a written recommendation and client approval before execution.', ar: 'جميع الإنفاق التسويقي يعمل تحت نموذج حوكمة محدد قبل نشر أي ميزانية. مؤشرات الأداء الرئيسية — بما في ذلك أهداف ROAS ومعايير CPL — متفق عليها كتابياً. التقارير الأسبوعية تقارن الأداء الفعلي بالأهداف حسب القناة. القنوات ضعيفة الأداء تُعدَّل بمبرر موثق. إعادة تخصيص الميزانية تتطلب توصية مكتوبة وموافقة العميل.' } },
    { q: { en: 'What happens if ROAS targets defined in the SOW are not met?', ar: 'ماذا يحدث إذا لم تتحقق أهداف ROAS المحددة في بيان العمل؟' }, a: { en: 'When ROAS falls below the agreed threshold at a scheduled review interval, an optimization protocol is triggered. The protocol includes audience performance analysis, bid strategy recalibration, creative rotation assessment, and a documented recommendation for budget reallocation. If the channel cannot meet benchmarks after two consecutive review cycles, an escalation path — defined in the SOW — governs whether the channel is paused or restructured.', ar: 'عندما ينخفض ROAS تحت العتبة المتفق عليها في فترة المراجعة المجدولة، يُفعَّل بروتوكول تحسين. يشمل تحليل أداء الجمهور وإعادة معايرة استراتيجية العطاء وتقييم تدوير الإبداعات وتوصية موثقة لإعادة تخصيص الميزانية. إذا لم تحقق القناة المعايير بعد دورتي مراجعة متتاليتين يحكم مسار التصعيد المحدد في بيان العمل.' } },
    { q: { en: 'How is marketing attribution validated across long B2B conversion cycles?', ar: 'كيف يُتحقق من إسناد التسويق عبر دورات تحويل B2B الطويلة؟' }, a: { en: 'B2B attribution requires multi-touch modeling with extended lookback windows. Rumuze configures GA4 and CRM data pipelines to track touchpoints across 30-, 60-, and 90-day attribution windows. Lead source is propagated through the CRM lifecycle from first-touch through closed deal, enabling revenue attribution at the campaign and audience level — not just the conversion level.', ar: 'إسناد B2B يتطلب نمذجة متعددة اللمس مع نوافذ نظرة للخلف ممتدة. روموز تضبط خطوط أنابيب بيانات GA4 وCRM لتتبع نقاط اللمس عبر نوافذ إسناد 30 و60 و90 يوماً. مصدر العميل المحتمل ينتشر عبر دورة حياة CRM من أول لمس حتى الصفقة المغلقة.' } },
    { q: { en: 'How does Rumuze prevent wasted ad spend in multi-channel campaigns?', ar: 'كيف تمنع روموز الإنفاق الإعلاني المهدر في الحملات متعددة القنوات؟' }, a: { en: 'Waste prevention is structural, not reactive. Campaign architecture is defined before any budget is deployed, with documented bid strategies and audience segmentation per channel. A/B tests enforce minimum statistical validity before creative changes. Weekly reporting identifies underperforming segments at the audience level — not just the campaign level. Budget is governed by protocol: no reallocation without documented analysis and client approval.', ar: 'منع الهدر هيكلي وليس تفاعلياً. معمارية الحملة محددة قبل نشر أي ميزانية مع استراتيجيات عطاء موثقة وتقسيم جمهور لكل قناة. اختبارات A/B تفرض حداً أدنى من الصلاحية الإحصائية قبل التغييرات الإبداعية. التقارير الأسبوعية تحدد الشرائح ضعيفة الأداء على مستوى الجمهور. الميزانية محكومة ببروتوكول: لا إعادة تخصيص دون تحليل موثق.' } },
  ],
  cta: { en: 'Organizations requiring governed performance marketing infrastructure with server-side attribution, structured campaign execution, and KPI-accountable delivery may initiate a scoped discovery engagement.', ar: 'المنظمات التي تتطلب بنية تحتية للتسويق الأدائي محكومة بإسناد من جانب الخادم وتنفيذ حملات منظم وتسليم خاضع لمساءلة مؤشرات الأداء الرئيسية يمكنها بدء ارتباط اكتشاف محدد النطاق.' },
};
